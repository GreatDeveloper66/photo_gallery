...
export default class dBConn {
    ...
    async connect(collectionName, payLoad, operation) {
        ...
        return await MongoClient.MongoClient.connect(process.env.MONGODB_LOCAL_CONN || process.env.MONGODB_REMOTE_CONN, { useNewUrlParser: true }, (err, client) => {
            ...
            switch (operation) {
                ...
                case 'delete':
                    ...
                    break;
                case 'find':
                    const found = collection.findOne(payLoad)
                    return found
                    break;
                default:
    ...
    findUser(payLoad) {
        this.connect(process.env.USER_COLLECTION, payLoad,'find')
    }
    ...
    findFavorite(payLoad) {
        this.connect(process.env.FAVORITES_COLLECTION, payLoad,'find')
    }
}
